...
def run_transcript_pressure_audit():
    audit_results = {
        "patch_id": "TOWER-ENGINE-050",
        "verdict": "FAIL",
        "checks": []
    }
    ...
    doc_path = os.path.join(project_root, "docs/design/console/combat_transcript_enemy_pressure_evidence.md")
    ...
    test_file_path = os.path.join(project_root, "engine/console/reports/tests/test_console_transcript_enemy_pressure.py")
    audit_results["checks"].append({
    ...
    try:
        with open(reporter_path, 'r') as f:
            content = f.read()
            required_fields = [
                "enemy_pressure_profiles_observed",
            ...
            patch_check = '"patch_id": "TOWER-ENGINE-050"' in content
            audit_results["checks"].append({
                "check": "patch_id updated to TOWER-ENGINE-050",
    ...
    try:
        test_result = subprocess.run(
            ["py", "-m", "pytest", "engine/console/reports/tests/test_console_transcript_enemy_pressure.py"],
            capture_output=True, text=True
        )
        audit_results["checks"].append({
            "check": "Console transcript enemy pressure integration tests pass",
    ...
    try:
        ...
        transcript = console_transcript_reporter.run_console_transcript(
            ["combat safe", "combat dangerous"], 
            debug=True, 
            transcript_id="enemy_pressure_validation"
        )
        
        # Verify file existence
        transcript_path = os.path.join(project_root, "outputs/console_transcripts/tower_engine_050_enemy_pressure_console_transcript.json")
        audit_results["checks"].append({
        ...
        if os.path.exists(transcript_path):
            with open(transcript_path, 'r') as f:
                data = json.load(f)
                content_ok = (
                    data.get("enemy_pressure_profiles_observed", 0) >= 1 and
                    len(data.get("enemy_archetypes_observed", [])) >= 1
                )
                audit_results["checks"].append({
                    "check": "Validation transcript contains enemy pressure evidence",
    ...
    audit_file_path = os.path.join(output_dir, "tower_engine_050_transcript_enemy_pressure_audit.json")
    ...
    result_file_path = os.path.join(result_dir, "tower_engine_050_transcript_enemy_pressure_result.json")
...
if __name__ == "__main__":
    audit = run_transcript_pressure_audit()
