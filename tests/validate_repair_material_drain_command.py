...
def run_repair_command_audit():
    audit_results = {
        "patch_id": "TOWER-ENGINE-068",
        "verdict": "FAIL",
        "checks": []
    }
    ...
    # 1. Check Files
    files_to_check = [
        "engine/console/runtime/tests/test_mvp_text_console_repair_materials.py",
        "docs/design/console/console_repair_material_drain_command.md"
    ]
    ...
    if os.path.exists(registry_path):
        with open(registry_path, 'r') as f:
            ...
            has_repair = any(cmd.get("command") == "repair" for cmd in registry.get("commands", []))
            audit_results["checks"].append({"check": "Console command registry includes repair", "status": "PASS" if has_repair else "FAIL"})
    ...
    try:
        with open(console_path, 'r') as f:
            content = f.read()
            handler_check = "def _handle_repair" in content
            dispatch_check = 'elif command == "repair":' in content
            trans_check = "inventory_transaction_stub.consume_inventory_item" in content
            
            audit_results["checks"].append({"check": "_handle_repair implemented", "status": "PASS" if handler_check else "FAIL"})
            audit_results["checks"].append({"check": "repair command dispatched", "status": "PASS" if dispatch_check else "FAIL"})
            audit_results["checks"].append({"check": "repair uses inventory stub", "status": "PASS" if trans_check else "FAIL"})
    ...
    try:
        test_result = subprocess.run(
            ["py", "-m", "pytest", "engine/console/runtime/tests/test_mvp_text_console_repair_materials.py"],
            capture_output=True, text=True
        )
        audit_results["checks"].append({
            "check": "Repair command unit tests pass",
    ...
    audit_file_path = os.path.join(output_dir, "tower_engine_068_repair_material_drain_command_audit.json")
    ...
    result_file_path = os.path.join(result_dir, "tower_engine_068_repair_material_drain_command_result.json")
...
if __name__ == "__main__":
    audit = run_repair_command_audit()
