...
def setup_teardown_test_dir():
    ...
    with open(os.path.join(project_root, "engine/floor_generation/identity/floor_identity_preservation_rules.json"), 'w') as f:
...
@pytest.fixture
def mock_replay_floor_diff_reporter():
    with patch('engine.reports.runtime.mvp_end_to_end_report._replay_floor_diff_reporter_available', True):
        with patch('engine.reports.runtime.mvp_end_to_end_report.replay_floor_diff_reporter') as mock_rfd:
            mock_rfd.make_replay_floor_diff_report.return_value = {
                "ok": True,
                "payload": {
            ...
            mock_rfd.write_replay_floor_diff_report.return_value = {"ok": True, "path": "/mock/path/diff_report.json"}
            yield mock_rfd


# --- Test run_mvp_end_to_end_report ---
def test_run_mvp_end_to_end_report_success(
    mock_startup_orchestrator, mock_scripted_simulation, mock_replay_floor_diff_reporter, setup_teardown_test_dir
):
